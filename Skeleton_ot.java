...
public class Skeleton<T>
{
    ...
    public T getSever(){return this.server;}
    public Class<T> getClass(){return this.sClass;}
    ...
    private void checkclass(Class<T> c){
        ...
        for(Method m:methods){
            ...
            ArrayList<?> exceptionList=new ArrayList(exceptions);
    ...
    {
        checkclass(c);
        if(server==null || server==null)throw new NullPointerException();
        this.sClass=c;
        this.sever=sever;
    }
    ...
    {
        checkclass(c);
        if(server==null||address==null)throw new NullPointerException();
        this.sClass=c;
        this.sever=sever;
        this.portNo=address.getPortNo();
    ...
    {
        if(running)throw new RMIException("Server is running now!");
        this.startRunning();
        try{
            listen_socket=new SeverSocket(this.portNo);
            this.portNo=listen_socket.getLocalPort();
            if(this.hostName==null)
                this.hostName=this.listen_socket.getInetAddress().getHostName();
            ...
            while(isRunning()){
                try{
                    ...
                    try{
                        sMethod=sClass.getMethod(methodName, parameterTypes);
                    }
                    catch(Exception e){
                    ...
                    if (sMethod){
                        try{
                            Object returnObject=sMethond.invoke(this.getServer(), args);
            ...
            System.out.println("Error: " + err.getMessage(), System.out);
            err.printStackTrace();
        }
    ...
    {
        if(!running)throw new RMIException("Server is not running now!");
        this.stopRunning();
        try{
            listen_socket.close();
        }catch(Throwable err){
